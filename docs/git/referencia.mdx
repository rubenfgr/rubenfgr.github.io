...
Se trae un commit por su referencia a tu posición actual. Tiene opciónes de --abort, --continue y --quit.
...
Nos movemos a la posición donde necesitamos traer el commit y ejecutamos el siguiente comando:
...
Solicitar un mensaje de confirmación antes de aplicar la operación cherry-pick

```sh
-edit
```

Mover el contenido sin realizar nueva confirmación

```sh
--no-commit
```

Añade una linea 'signoff' al final del mensaje de confirmación de cherry-pick
